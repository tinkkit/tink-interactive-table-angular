...
;angular.module('tink.interactivetable').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('templates/tinkTableAction.html',
    ...
    "<span>{{action.name}}</span> </li> <hr> <li data-ng-repeat=\"action in tinkActions | orderBy:'+order' | filter: { master: false }\" ng-disabled=\"checked().length === 0 || (action.single && checked().length > 1)\" data-ng-click=actionCallBack(action)> <i class=\"fa {{action.icon}} fa-fw\"></i>\n" +
